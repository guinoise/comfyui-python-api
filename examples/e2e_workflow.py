"""
End 2 end example using a standard workflow (not an api workflow) to query ComfyUI server
"""
import logging
import sys
...
logger= helper_logger.getChild("e2e_workflow")
...
from comfyui_utils.workflow2prompt import Workflow

async def run_standard_workflow(address: str, user_string: str, workflow_path: pathlib.Path, output_path: pathlib.Path=None, sample_dir: pathlib.Path=None):
    comfyui = comfy.ComfyAPI(address)

    # Load the standard worflow format into the utility object for conversion
    workflow= await Workflow.load(path=workflow_path, comfyapi_client=comfyui)
    out_api= workflow_path.parent.joinpath(f"{workflow_path.stem}_api.json")
    await workflow.save_api(out_api)
    api= await workflow.generate_api_workflow()
    wf= workflow._data
    ...
    try:
        extra_data= {"extra_pnginfo": wf}
        await comfyui.submit(api, Callbacks(), extra_data)
    ...
    if output_path is not None:
        if "images" not in result["output"]:
            logger.error("Images not in the results: %r", result["output"])
        else:            
...
def main():
    parser = argparse.ArgumentParser(description='Run the default ComfyUI workflow or other workflow on a deployed ComfyUI server using a standard workflow, not the API workflow.')
    ...
    parser.add_argument("--workflow_path", type=pathlib.Path, help="Standard workflow to load. Default to examples/workflows/default_workflow.json", default=pathlib.Path('./examples/workflows/default_workflow.json'))
    ...
    asyncio.run(run_standard_workflow(args.address, args.prompt, args.workflow_path, outfile, args.sample_dir))
